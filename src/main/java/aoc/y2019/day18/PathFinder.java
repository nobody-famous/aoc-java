...
public class PathFinder {
    ...
    public int find() {
        ...
        var entrances = new ArrayList<Point>(grid.getEntrances());
        return traverse(entrances, 0, 0);
        // for (var entrance : grid.getEntrances()) {
    ...
    private int traverse(List<Point> points, int lastStep, int foundKeys) {
        if ((foundKeys & allKeysMask) == allKeysMask) {
            return lastStep;
        }

        System.out.println("traverse " + points);
        for (var pt : points) {
        ...
        var shortest = Integer.MAX_VALUE;
        for (var ndx = 0; ndx < points.size(); ndx += 1) {
            var pt = points.get(ndx);
            var distKeysMap = distances.get(pt);
            ...
            for (var keyDist : candidates.values()) {
                ...
                var newPoints = new ArrayList<Point>(points);

                newPoints.set(ndx, newPt);
                var dist = traverse(newPoints, keyDist.dist(), newKeys);
